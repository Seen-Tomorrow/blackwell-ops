...
pub struct AtomcodeEngineRef {
    ...
    /// Seat launched with mmproj — enables supports_vision on this provider.
    #[serde(default)]
    pub vision: bool,
}
...
fn build_config_toml(req: &AtomcodeLaunchRequest) -> Result<String, String> {
    ...
    match mode.as_str() {
        "solo" => {
            ...
            // vision_preprocessor only when this seat has mmproj (AtomCode image_url path).
            let vision_pre = if primary_vision {
                format!("vision_preprocessor_provider = \"{prov}\"\n", prov = PROVIDER_SOLO)
            } else {
                String::new()
            };
            Ok(format!(
...
context_window = {ctx}
capable_model = 2
# Native multimodal only when engine launched with mmproj.
...
{webui_tail}"#,
                prov = PROVIDER_SOLO,
                vision_pre = vision_pre,
                model = toml_escape(&primary_model),
                url = toml_escape(&primary_url),
                ctx = primary_ctx,
                vision = if primary_vision { "true" } else { "false" },
        ...
        "brain_workers" | "brain+workers" | "dual" => {
            ...
            let worker_vision = worker.vision;
            ...
            let n = concurrent;
            // Prefer brain for vision preprocessor when it has mmproj; else worker.
            let vision_pre = if primary_vision {
                format!(
                    "vision_preprocessor_provider = \"{prov}\"\n",
                    prov = PROVIDER_BRAIN
                )
            } else if worker_vision {
                format!(
                    "vision_preprocessor_provider = \"{prov}\"\n",
                    prov = PROVIDER_WORKER
                )
            } else {
                String::new()
            };
...
{webui_tail}"#,
                ...
                brain_vision = if primary_vision { "true" } else { "false" },
                worker_model = toml_escape(&worker_model),
                worker_url = toml_escape(&w_url),
                worker_vision = if worker_vision { "true" } else { "false" },
                worker_ctx = w_ctx,
                n = n,
                webui_tail = webui_tail,
            ))
