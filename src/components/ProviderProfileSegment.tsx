...
export type ProviderProfileSegmentOption = {
  id: string;
  label: ReactNode;
  title?: string;
  disabled?: boolean;
...
export interface ProviderProfileSegmentProps {
  options: ProviderProfileSegmentOption[];
  selectedId: string;
  ...
  disabled?: boolean;
  /**
   * `fit` — provider/profile bezel scale.
   * `compact` — smaller scale.
   */
  size?: "fit" | "compact";
  /**
   * `accent` — theme accent thumb.
...
/**
 * DEDICATED provider + binary-profile selector segmented switch.
 *
 * Forked from the shared measured-thumb core so the provider/profile bar can
 * evolve independently (build-state animations, profile gating, future
 * provider features) without interfering with — or being constrained by —
 * the other segment instances (header nav, cockpit flags, GPU bezel,
 * config toggles).
 *
 * Do NOT reuse this for non-provider segments; use `SegmentSwitch` for those.
 */
export default function ProviderProfileSegment({
  options,
  selectedId,
  onSelect,
  ariaLabel,
